...
class Soal extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->ion_auth->is_admin()) {
            //Jika admin maka tampilkan semua matpel
            $data['matpel'] = $this->master->getAllMatpel();
        } else {
            //Jika bukan maka matpel dipilih otomatis sesuai matpel guru
            $data['matpel'] = $this->soal->getMatpelGuru($user->username);
        }
    ...
    public function add()
    {
        ...
        if ($this->ion_auth->is_admin()) {
            ...
            //Jika bukan maka matpel dipilih otomatis sesuai matpel guru
            $data['guru'] = $this->soal->getMatpelGuru($user->username);
        }
    ...
    public function save()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $data = [
                'soal'      => str_replace(
                    "<p data-f-id=\"pbf\" xss=removed>Powered by <a href=\"https://www.froala.com/wysiwyg-editor?pb=1\" title=\"Froala Editor\">Froala Editor</a></p>",
                    "",
                    $this->input->post('soal', true)
                ),
            ...
            foreach ($abjad as $abj) {
                $data['opsi_' . $abj]    = str_replace(
                    "<p data-f-id=\"pbf\" xss=removed>Powered by <a href=\"https://www.froala.com/wysiwyg-editor?pb=1\" title=\"Froala Editor\">Froala Editor</a></p>",
                    "",
                    $this->input->post('jawaban_' . $abj, true)
                );
            }
