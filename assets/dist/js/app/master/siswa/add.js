function load_jenjangkelas() {
    $('#jenjangkelas').find('option').not(':first').remove();

    $.getJSON(base_url+'jenjangkelas/load_jenjangkelas', function (data) {
        var option = [];
        for (let i = 0; i < data.length; i++) {
            option.push({
                id: data[i].id_jenjangkelas,
                text: data[i].nama_jenjangkelas
            });
        }
        $('#jenjangkelas').select2({
            data: option
        })
    });
}
...
function load_kelas(id) {
    ...
    $.getJSON(base_url+'kelas/kelas_by_jenjangkelas/' + id, function (data) {
        var option = [];
        for (let i = 0; i < data.length; i++) {
            option.push({
                id: data[i].id_kelas,
...
$(document).ready(function () {
    ...
    // Load Kelas By Jenjangkelas
    $('#jenjangkelas').on('change', function () {
        load_kelas($(this).val());
    });
