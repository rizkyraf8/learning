...
$(document).ready(function () {
    if (banyak < 1 || banyak > 50) {
        ...
        window.location.href = base_url+"jenjangkelas";
    } else {
        generate(banyak);
    }

    $('#inputs input:first').select();
    $('form#jenjangkelas input').on('change', function () {
        $(this).parent('.form-group').removeClass('has-error');
        $(this).next('.help-block').text('');
    });

    $('form#jenjangkelas').on('submit', function (e) {
        e.preventDefault();
            ...
            <tr>
                ...
                <td>
                    <div class="form-group">
                        <input autocomplete="off" type="text" name="nama_jenjangkelas[${i}]" class="input-sm form-control">
